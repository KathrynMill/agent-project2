...
class LinuxController(BaseController):
    ...
    def _setup_audio_control(self):
        ...
        try:
            ...
            if result.returncode == 0:
                self.audio_control = 'pactl'
            else:
                # 尝试alsamixer
                result = subprocess.run(['which', 'amixer'], capture_output=True, text=True)
                if result.returncode == 0:
                    self.audio_control = 'amixer'
                else:
                    self.audio_control = None
                    logger.warning("No audio control tool found (pactl or amixer)")
    ...
    async def _volume_up(self, amount: int) -> CommandResult:
        ...
        try:
            if self.audio_control == 'pactl':
                cmd = ['pactl', 'set-sink-volume', '@DEFAULT_SINK@', f'+{amount}%']
            elif self.audio_control == 'amixer':
                cmd = ['amixer', 'set', 'Master', f'{amount}%+']
            else:
                return CommandResult(
                    success=False,
    ...
    async def _screenshot(self, save_path: str = None) -> CommandResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return CommandResult(
                    success=False,
                    message=f"Screenshot failed: {stderr.decode()}",
                    execution_time=time.time() - start_time
                )
        except Exception as e:
            return CommandResult(
