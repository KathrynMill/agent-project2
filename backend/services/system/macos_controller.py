...
class MacOSController(BaseController):
    ...
    def _setup_audio_control(self):
        """设置音频控制"""
        try:
            # macOS使用osascript进行音频控制
            self.audio_control = 'osascript'
            logger.info("Audio control method: osascript")
    ...
    async def _volume_up(self, amount: int) -> CommandResult:
        ...
        try:
            # 使用osascript控制音量
            script = f"set volume output volume (output volume of (get volume settings) + {amount})"
